 ...
 * that, call {@link GlSurfaceTexture#setAspectRatioMode(int)} with
 * {@link GlSurfaceTexture#ASPECT_RATIO_PREVIEW} after creating your
 * {@link GlSurfaceTexture}. <br />
 */
public class GlSurfaceTexture extends SurfaceTexture implements Runnable, OnFrameAvailableListener, SurfaceHolder.Callback {
    public final static String TAG = GlSurfaceTexture.class.getSimpleName();
    ...
    public GlSurfaceTexture(int texName) {
        super(texName);
        init();
    }

    public GlSurfaceTexture(int texName, boolean singleBufferMode) {
        super(texName, singleBufferMode);
        init();
    }

    @TargetApi(Build.VERSION_CODES.O)
    public GlSurfaceTexture(boolean singleBufferMode) {
        super(singleBufferMode);
        init();
    }

    private void init() {
        getSurfaceTexture().addCallback(this);
    }
    ...
    public void startGLThread() {
        ...
        if (mTextureManager.getSurfaceTexture() == null) {
            mThread = new Thread(GlSurfaceTexture.this);
            mRunning = true;
            mThread.start();
